...
function Profil() {
    ...
    useEffect(() => {
        const canvas = document.getElementById('canvaProfil');
        const ctx = canvas.getContext('2d');

        const originalImage = new Image();
        originalImage.src = utilisateur.photo;

        originalImage.onload = function() {
            canvas.width = originalImage.width; // Définissez la largeur du canvas sur la largeur de l'image
            canvas.height = originalImage.height; // Définissez la hauteur du canvas sur la hauteur de l'image
          
            ctx.drawImage(originalImage, 0, 0); // Dessinez l'image d'origine aux coordonnées (0, 0) du canvas
        };
    }, [])
    ...
    function editPhoto(newPhoto) {
        utilisateur = { ...utilisateur, photo: newPhoto };
            ...
            <div className="block_general">
                ...
                <div className="block_photo_nom_tot">
                    ...
                    <div className="photo_nom">
                        <canvas id="canvaProfil" ref={canvasRef} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} src={utilisateur.photo} >
