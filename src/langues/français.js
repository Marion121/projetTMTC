

export const français = {
    ACCUEIL: {
        inscription: "Inscription",
        connexion: "Se connecter",
        achetez: "Achetez",
        voyagez: "Voyagez",
        recevez: "Recevez",
        texte: "Texte"
    },
    NAVBAR: {
        creer: "Creer une annonce",
        annonces: "Annonces",
        mesAnnonces: "Mes annonces",
        mesAchats: "Mes achats",
        lesAnnonces: "Les Annonces",
        profil: "Mon profil",
        deconnecter: "Se déconnecter"
    },
    CONNEXION:{
        connexion: "Connexion",
        email: "Email",
        emailPH: "Entrer votre e-mail",
        mdp: "Mot de passe",
        mdpPH: "Entrer le mot de passe"
    },
    PROFIL: {
        total: "Total gagné",
        mail: "Email",
        dateNaissance: "Date de Naissance",
        pays: "Pays de résidence", 
        CoordoneesBanque: "Coordonées bancaires"
    },
    ANNONCES: {
        bonjour: "Bonjour", 
        textEntete_1: "Retrouvez si dessous l'ensemble des annonces postées !",
        textEntete_2: "Postulez si vous avez de la place dans votre valise ! ",
        depart: "Depart :",
        arrivee: "Arrivee :",
        produit: "Produit",
        electronique: "Electronique",
        comestible: "Comestible",
        liquide: "Liquide",
        typeAnnonce: "Type d'annonce",
        achat: "Achat",
        voyage: "Voyage", 
        retribution: "Retribution",
        devise: "Devise",
        annoncesUrg: "Annonces Urgentes",
        motsCles: "Chercher des mots clés"
    },
    DETAIL_ANNONCE: {
        descriptionAstuce: "Le Lorem Ipsum est simplement du faux texte employé dans la composition et la mise en page avant impression. Le Lorem Ipsum est le faux texte standard de l'imprimerie depuis les années 1500, quand un imprimeur anonyme assembla ensemble des morceaux de texte pour réaliser un livre spécimen de polices de texte. ",
        titreAstuce: "Astuce Bwob",
        acheterLivrer: "Je peux acheter et livrer le produit",
        acheter: "Je peux acheter",
        livrer: "Je peux livrer"
    },
    COMPONENT_ANNONCE:{
        achat: "Achat",
        voyage: "Voyage",
        contrepartie: "Contrepartie",
        coutProduit: "Cout du produit"
    },
    MES_ANNONCES: {
        titre: "Mes Annonces",
        detailsPrix: "Détails du prix",
        total: "Total",
        etat: "Etat",
        attenteVoyageurAcheteur: "En attente d'un voyageur et d'un acheteur...",
        attenteAcheteur: "En attente d'un acheteur...",
        attenteVoyageur: "En attente d'un voyageur...",
        attentePayement: "En attente du payement...",
        modePayement: "Mode de payement",
        modeCarte: "Par carte",
        modeEspece: "En espèce",
        produitAchete: "Produit acheté",
        enCours: "En cours d'acheminement",
        livré: "Livré"
    },
    MES_ACHATS_VOYAGES:{
        contrepartie: "Ma contrepartie",
        actionAttente: "Actions en attente",
        actionAcheter: "Acheter le produit",
        actionLivraison: "En cours de livraison"
    },
    CREER_ANNONCE_G:{
        precedent: "Precedent",
        suivant: "Suivant",
        detailProduit: "Detail du produit",
        prixProduit: "Prix d'acheminement du produit",
        recapitulatif: "Recapitulatif de votre annonce"
    },
    CREER_ANNONCE_1: {
        titreLieu: "Lieu d'achat et de Réception",
        paysAnnonce : "Pays de votre produit",
        villeArrivee: "Ville de livraison de votre produit",
        titreDetail: "Détails du produit",
        photoProduit: "Photo de votre produit", 
        glisser: "Gisser ici",
        poid: "Poids du produit",
        titreAnnonce: "Titre de l'annonce",
        description: "Description"
    },
    CREER_ANNONCE_2:{
        titre: "Prix et acheminement du produit",
        prix: "Prix d'achat du produit",
        devise: "Devise",
        degImportance: "Degrés d'importance",
        degImportanceN: "Normal",
        degImportanceU: "Urgent",
        acheminement: "Acheminement du produit",
        cliquerIci: "cliquez ici pour selectionner",
        acheteur: "J'ai besoin d'un acheteur :",
        contrepartie: "Contrepartie à votre charge :",
        voyageur: "J'ai besoin d'un voyageur :"
    },
    CREER_ANNONCE_3: {
        titrePage: "Récapitulatif de votre annonce",
        titreAnnonce: "Titre de l'annonce",
        paysAchat: "Pays d'achat",
        villeLivraison: "Ville de livraison",
        prixProduit: "Prix du produit",
        description: "Description",
        commissionA: "Commission de l'acheteur",
        commissionV: "Commission du voyageur",
        commissionBwob: "Commission de BWob",
        prixTot: "Prix total",
        texte: "Cette somme sera à régler ..."
    }
};